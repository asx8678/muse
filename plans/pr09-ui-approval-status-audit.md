...
`/approve plan` previously used wording that implied the plan could move toward implementation immediately after approval. That was dangerously easy to read as “approval automatically triggers implementation.” Approval transitions plan/session state only; it does not start a runner or implementation turn. The UI copy now says approval records the plan decision only and implementation still requires a later explicit gate.
...
Updated `/approve plan` success copy so it does not imply approval triggers implementation.

## Safety notes

- No plan lifecycle command now triggers implementation; tests assert `active_turn_id == nil` and `runner_pid == nil` after approve/reject.

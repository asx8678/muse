...
- `test/muse/session_server_test.exs`
  ...
  - auditable lifecycle event assertions
  - non-executing boundary checks (no turn execution side-effects)
- Approval binding / stale checks (PR09 lanes)
  - content-bound approval identity (`session_id`, `plan_id`, `plan_version`, `plan_hash`, `workspace`)
...
- [ ] Plan lifecycle emits auditable events (`:plan_approved`/`:plan_rejected` + status change when applicable)

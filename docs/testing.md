...
- `test/muse/pr17_patch_proposal_test.exs`
  - session transition to `:awaiting_patch_approval` is valid
  - approval `:patch` kind with `patch_id`/`patch_hash` fields
  - content-bound patch identity (stale patch approval rejection)
  - patch approval does not apply files, create checkpoints, or run shell/network commands
  - `patch_propose` blocked for Planning Muse; available only after plan approval for Coding Muse
...
**Gaps:** Full end-to-end patch proposal flow (Coding Muse → `patch_propose` tool → `:awaiting_patch_approval` → `/approve patch`) requires Conductor Coding Muse routing and `patch_propose` tool implementation, which are not yet wired. The PR17 test exercises data model contracts and boundary checks only; the E2E flow will be added when runtime wiring lands.

defmodule Muse.RuntimeProvider do
  ...
  (`MUSE_PROVIDER`, `MUSE_MODEL`, etc.) and the Conductor's opts-based provider
  resolution. It is the single point where LiveView and other web callers
  decide whether to route submits through the configured provider or keep
  the default fake/offline behavior.
  ...
  ## Behavior by environment
    ...
    * **dev / prod with `MUSE_PROVIDER=fake` or unset** — returns `{:ok, []}`,
      using the Conductor's default fake provider.
    * **dev / prod with a valid non-fake provider** — returns
      `{:ok, [provider_env: filtered_env, model_router_opts: [env: filtered_env]]}`,
  ...
  Returns:
    ...
    * `{:ok, keyword()}` — opts to pass to `Muse.submit/3` or
      ...
      (fake) provider. Non-empty list contains `:provider_env` and
      `:model_router_opts` for Conductor resolution.
  ...
  defp resolve_runtime_opts do
    ...
    provider = Map.get(env_map, "MUSE_PROVIDER", "fake")

    if provider == "fake" do
      {:ok, []}
    else
      resolve_non_fake_provider(env_map, provider)
    end
  end

  defp resolve_non_fake_provider(env_map, _provider) do
    case Config.llm_provider_config(env_map) do
      {:ok, %ProviderConfig{} = config} ->
        if ProviderConfig.provider_atom(config) == :fake do
          # Resolved back to fake despite non-fake MUSE_PROVIDER —
          # this shouldn't happen with valid config, but handle it safely.
          {:ok, []}
        else
          # Valid non-fake provider — pass filtered env for
          # Conductor resolution and per-Muse model routing.
          filtered = filter_muse_env(env_map)
          {:ok, [provider_env: filtered, model_router_opts: [env: filtered]]}
        end

defmodule Muse.PlanParser do
  ...
  defp extract_json(text, :strict), do: {:ok, String.trim(text)}
  ...
  defp extract_json(text, :auto) do
    ...
    case decode_json(trimmed) do
      {:ok, _decoded} -> {:ok, trimmed}
      {:error, _errors} -> try_fenced_then_prose(text)
    end
  end

  defp extract_json(text, _unknown), do: extract_json(text, :strict)
